...
btnsEl.forEach(btn => btn.addEventListener('click', handleClick))
...
function handleClick(clickedButtonEvent) {
    ...
    const txtOnBtn = cardNode.querySelector('button').textContent
    console.log(txtOnBtn)

    if (txtOnBtn === 'Подробнее') { // 4.1 проверяем равняется ли этот текст строке "Подробнее"
        // 4.2 если да, то передаем объект card в функцию showMoreText
        showMoreText(card)
    } else if (txtOnBtn === 'Отмена') { // 4.3 проверяем равняется ли текст на кнопке строке "Отмена"
...
function hideMoreText(card) {
    ...
    if(cardNode.children.classList.contains('desk')) {
        cardNode.children.classList.remove('desk')
    }
